...
cd "cryptodezirecash-$HOST" || (echo "could not enter distdir cryptodezirecash-$HOST"; exit 1)
...
if [ "$RUN_BENCH" = "true" ]; then
  BEGIN_FOLD bench
  DOCKER_EXEC LD_LIBRARY_PATH=$TRAVIS_BUILD_DIR/depends/$HOST/lib $OUTDIR/bin/bench_cryptodezirecash -scaling=0.001
  END_FOLD
fi
